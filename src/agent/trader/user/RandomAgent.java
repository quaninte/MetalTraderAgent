...
public class RandomAgent extends UserAgent {
			...
			try {
				...
				if (action.getAction() instanceof WantTo) {
					...
					if (wantTo.getGood().getMetal().equals(Metal.PLATINUM)) {
						proposedBid = mPlaPrice * (1 + (RANDOM.nextDouble() * 2 - 1) * 0.2) * wantTo.getGood().getQuantity();
					} else if (wantTo.getGood().getMetal().equals(Metal.GOLD)) {
						proposedBid = mGolPrice * (1 + (RANDOM.nextDouble() * 2 - 1) * 0.2) * wantTo.getGood().getQuantity();
					} else if (wantTo.getGood().getMetal().equals(Metal.SILVER)) {
						proposedBid = mSilPrice * (1 + (RANDOM.nextDouble() * 2 - 1) * 0.2) * wantTo.getGood().getQuantity();
					} else {
